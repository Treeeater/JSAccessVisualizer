...
exports.loadExistingPolicies = function(msg){
	var tpd = msg.tpd;
	var hd = msg.hd;
	var filePath = rootPolicyPath + "extra\\" + hd + "\\" + tpd + ".txt";
	var genericPath = rootPolicyPath + "extra\\" + hd + "\\generic.txt";
	var basePath = rootPolicyPath + "\\..\\..\\policies\\" + tpd + ".txt";
	var existingPoliciesLoaded = "";
	var basePoliciesLoaded = "";
	if (checkFileExistence(filePath)) {
		existingPoliciesLoaded += readFromFile(filePath);
		if (existingPoliciesLoaded[existingPoliciesLoaded.length-1] != "\n") existingPoliciesLoaded += "\n";
	}
	if (checkFileExistence(genericPath)) {
		existingPoliciesLoaded += readFromFile(genericPath);
	}
	if (checkFileExistence(basePath)) {
		basePoliciesLoaded = readFromFile(basePath);
	}
	sideBarWorker.port.emit("existingPoliciesLoaded", {tpd:tpd, hd:hd, existingPoliciesLoaded:existingPoliciesLoaded, basePoliciesLoaded:basePoliciesLoaded});
}
...
exports.reportViolatingDomains = function(data){
	sideBarWorker.port.emit("reportViolatingDomains", data);
}

exports.postToInteractive = function(data){
	sideBarWorker.port.emit("postToInteractive", data);
}
...
exports.setPolicyPath = function (r){
	console.log("Using policy directory: " + r);
	rootPolicyPath = r;
}
...
exports.changedOvermatchThreshold = function(msg){
	sideBarWorker.port.emit("changedOvermatchThreshold", msg);
}

exports.changedTagThreshold = function(msg){
	sideBarWorker.port.emit("changedTagThreshold", msg);
}
...
{
	if (!!fileName) {
		var fn = rootPolicyPath + fileName;
		initDir(fn);
